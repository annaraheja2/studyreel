        ...
        <div className="max-w-5xl mx-auto px-5 h-16 flex items-center gap-4">
          <Link to="/" className="shrink-0 flex items-center gap-2.5">
            <span className="relative grid place-items-center w-7 h-7 rounded-full bg-[#1F2A36] overflow-hidden">
              <span className="absolute inset-0 rounded-full ring-2 ring-[#D98A2B]/70" />
              <span className="absolute w-4 h-4 rounded-full bg-[#D98A2B] translate-x-1.5 -translate-y-0.5" />
            </span>
          ...
          <nav className="flex items-center gap-4 text-sm shrink-0">
            ...
            {!user ? (
              <Link
                to="/login"
                className="px-4 py-1.5 rounded-md bg-[#B5610F] text-[#FBF3E7] font-semibold hover:bg-[#9C5410] transition"
              >
                Sign in
              </Link>
            ) : (
