...
export default function AdminCourse() {
  ...
  if (!isAdmin) return (
    <div className="max-w-md mx-auto mt-10 text-center space-y-3">
      ...
      <Link to="/" className="inline-block text-[#2F6FB0]">← Home</Link>
    </div>
  )
  if (!draft) return <p className="text-[#74828F]">Loading course…</p>
  ...
  const input = 'w-full px-3 py-2 rounded-lg bg-[#E9F2FB] border border-[#CADDEE] text-[#1F2A36] placeholder-[#93A2B0] focus:border-brand-400 focus:ring-1 focus:ring-brand-500/40 outline-none text-sm'

  return (
    <div className="space-y-5 pb-24">
      <Link to="/admin" className="inline-flex items-center gap-1.5 text-sm text-[#74828F] hover:text-[#1F2A36] transition"><ArrowLeft className="w-4 h-4" /> Admin</Link>

      {/* Course details */}
      <div className="bg-[#FBFDFF] rounded-2xl border border-[#CADDEE] p-5 space-y-3">
      ...
      {draft.units.map((unit, ui) => (
        <div key={unit.id} className="bg-[#FBFDFF] rounded-2xl border border-[#CADDEE] p-4">
          <div className="flex items-center gap-2">
            <button onClick={() => toggle(unit.id)} className="text-[#74828F] w-5">{open[unit.id] ? '▾' : '▸'}</button>
            ...
            <button onClick={() => edit((c) => { c.units.splice(ui, 1) })} className="text-[#74828F] hover:text-red-600 transition px-2"><Trash className="w-4 h-4" /></button>
          </div>
          ...
          {open[unit.id] && (
            <div className="mt-3 pl-6 space-y-3">
              ...
              {unit.lessons.map((lesson, li) => (
                <div key={lesson.id} className="border border-[#CADDEE] rounded-xl p-3">
                  <div className="flex items-center gap-2">
                    <button onClick={() => toggle(lesson.id)} className="text-[#74828F] w-5">{open[lesson.id] ? '▾' : '▸'}</button>
                    ...
                    <button onClick={() => edit((c) => { c.units[ui].lessons.splice(li, 1) })} className="text-[#74828F] hover:text-red-600 transition px-2"><Trash className="w-4 h-4" /></button>
                  </div>
                  ...
                  {open[lesson.id] && (
                    <div className="mt-3 space-y-2">
                      ...
                      <label className="block text-xs font-medium text-[#566573]">Video — drag a file to upload, or paste a link (Google Drive / YouTube / .mp4)</label>
                      ...
                      <div className="flex gap-2">
                        <div className="flex-1">
                          <label className="block text-xs font-medium text-[#566573]">Difficulty (1–5)</label>
                        ...
                        <div className="flex-1">
                          <label className="block text-xs font-medium text-[#566573]">Tags (comma-separated)</label>
                      ...
                      {/* Questions */}
                      <div className="mt-2">
                        <div className="text-xs font-semibold text-[#566573] mb-1">Quiz questions</div>
                        {lesson.questions.map((qq, qi) => (
                          <div key={qi} className="bg-[#EDF5FC] rounded-lg p-3 mb-2 space-y-2">
                            <div className="flex gap-2">
                              ...
                              <button onClick={() => edit((c) => { c.units[ui].lessons[li].questions.splice(qi, 1) })} className="text-[#74828F] hover:text-red-600 transition px-1"><Trash className="w-4 h-4" /></button>
                            </div>
                            ...
                            <div className="text-xs text-[#74828F]">Select the radio button next to the correct answer.</div>
                          </div>
                        ))}
                        <button onClick={() => edit((c) => { c.units[ui].lessons[li].questions.push(newQuestion()) })}
                          className="text-sm text-[#2F6FB0]">+ Add question</button>
                      </div>
                    </div>
                  )}
                </div>
      ...
      <button
        ...
        className="text-sm px-3 py-2 rounded-lg border border-[#B9D2E8] hover:bg-black/[0.05] text-[#1F2A36]"
      >+ Add unit</button>

      {/* Sticky save bar */}
      <div className="fixed bottom-0 left-0 right-0 bg-[#C9E3F7]/90 backdrop-blur border-t border-[#CADDEE] p-3">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <span className="text-sm text-[#566573]">{dirty ? '● Unsaved changes' : 'All changes saved'}</span>
          <button onClick={save} disabled={!dirty || saving}
            className="px-5 py-2 rounded-lg bg-[#B5610F] text-[#FBF3E7] font-semibold hover:bg-[#9C5410] transition disabled:opacity-50">
