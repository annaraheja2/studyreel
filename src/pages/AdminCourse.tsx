...
export default function AdminCourse() {
  ...
  if (!isAdmin) return (
    <div className="max-w-md mx-auto mt-10 text-center space-y-3">
      ...
      <Link to="/" className="inline-block text-[#9A6A45]">← Home</Link>
    </div>
  )
  if (!draft) return <p className="text-[#8A8071]">Loading course…</p>
  ...
  const input = 'w-full px-3 py-2 rounded-lg bg-[#F4EEE3] border border-[#E1D8C8] text-[#2B2620] placeholder-[#A99E8D] focus:border-brand-400 focus:ring-1 focus:ring-brand-500/40 outline-none text-sm'

  return (
    <div className="space-y-5 pb-24">
      <Link to="/admin" className="inline-flex items-center gap-1.5 text-sm text-[#8A8071] hover:text-[#2B2620] transition"><ArrowLeft className="w-4 h-4" /> Admin</Link>

      {/* Course details */}
      <div className="bg-[#FBF8F2] rounded-2xl border border-[#E1D8C8] p-5 space-y-3">
      ...
      {draft.units.map((unit, ui) => (
        <div key={unit.id} className="bg-[#FBF8F2] rounded-2xl border border-[#E1D8C8] p-4">
          <div className="flex items-center gap-2">
            <button onClick={() => toggle(unit.id)} className="text-[#8A8071] w-5">{open[unit.id] ? '▾' : '▸'}</button>
            ...
            <button onClick={() => edit((c) => { c.units.splice(ui, 1) })} className="text-[#8A8071] hover:text-red-600 transition px-2"><Trash className="w-4 h-4" /></button>
          </div>
          ...
          {open[unit.id] && (
            <div className="mt-3 pl-6 space-y-3">
              ...
              {unit.lessons.map((lesson, li) => (
                <div key={lesson.id} className="border border-[#E1D8C8] rounded-xl p-3">
                  <div className="flex items-center gap-2">
                    <button onClick={() => toggle(lesson.id)} className="text-[#8A8071] w-5">{open[lesson.id] ? '▾' : '▸'}</button>
                    ...
                    <button onClick={() => edit((c) => { c.units[ui].lessons.splice(li, 1) })} className="text-[#8A8071] hover:text-red-600 transition px-2"><Trash className="w-4 h-4" /></button>
                  </div>
                  ...
                  {open[lesson.id] && (
                    <div className="mt-3 space-y-2">
                      ...
                      <label className="block text-xs font-medium text-[#6E6459]">Video — drag a file to upload, or paste a link (Google Drive / YouTube / .mp4)</label>
                      ...
                      <div className="flex gap-2">
                        <div className="flex-1">
                          <label className="block text-xs font-medium text-[#6E6459]">Difficulty (1–5)</label>
                        ...
                        <div className="flex-1">
                          <label className="block text-xs font-medium text-[#6E6459]">Tags (comma-separated)</label>
                      ...
                      {/* Questions */}
                      <div className="mt-2">
                        <div className="text-xs font-semibold text-[#6E6459] mb-1">Quiz questions</div>
                        {lesson.questions.map((qq, qi) => (
                          <div key={qi} className="bg-[#F7F1E7] rounded-lg p-3 mb-2 space-y-2">
                            <div className="flex gap-2">
                              ...
                              <button onClick={() => edit((c) => { c.units[ui].lessons[li].questions.splice(qi, 1) })} className="text-[#8A8071] hover:text-red-600 transition px-1"><Trash className="w-4 h-4" /></button>
                            </div>
                            ...
                            <div className="text-xs text-[#8A8071]">Select the radio button next to the correct answer.</div>
                          </div>
                        ))}
                        <button onClick={() => edit((c) => { c.units[ui].lessons[li].questions.push(newQuestion()) })}
                          className="text-sm text-[#9A6A45]">+ Add question</button>
                      </div>
                    </div>
                  )}
                </div>
      ...
      <button
        ...
        className="text-sm px-3 py-2 rounded-lg border border-[#D8CCB9] hover:bg-black/[0.05] text-[#2B2620]"
      >+ Add unit</button>

      {/* Sticky save bar */}
      <div className="fixed bottom-0 left-0 right-0 bg-[#ece5d8]/90 backdrop-blur border-t border-[#E1D8C8] p-3">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <span className="text-sm text-[#6E6459]">{dirty ? '● Unsaved changes' : 'All changes saved'}</span>
          <button onClick={save} disabled={!dirty || saving}
            className="px-5 py-2 rounded-lg bg-[#2B2620] text-[#F4EFE6] font-semibold hover:bg-[#3D352B] transition disabled:opacity-50">
